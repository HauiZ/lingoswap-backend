...
const getUserById = async (req, res) => {
  try {
    ...
    logger.info(`Xem profile public user ID: ${id}`);
    res.json(user);
  } catch (error) {
...
const updateMyProfile = async (req, res) => {
  try {
    ...
    logger.info(`Cập nhật hồ sơ cá nhân thành công ID: ${req.user.id}`);
...
const uploadAvatar = async (req, res) => {
  try {
    ...
    logger.info(`Cập nhật avatar thành công ID: ${req.user.id}`);
